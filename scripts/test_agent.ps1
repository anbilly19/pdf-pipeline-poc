...
retriever = BBoxRetriever(store=store, embedder=embedder, top_k=15)
agent = build_agent(retriever=retriever, provider="ollama", model="qwen2.5:3b")

query = "Welche Fristen gelten bei Verzug und was kann der Auftraggeber verlangen?"
print(f"\nQuery: {query}\n")
