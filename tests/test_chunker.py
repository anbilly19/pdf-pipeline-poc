...
def test_table_is_own_chunk() -> None:
    chunker = LayoutChunker()
    page = _make_page([_text("Vor der Tabelle."), _table("| A | B |\n| 1 | 2 |"), _text("Nach der Tabelle.")])
    chunks = chunker.chunk([page])
...
def test_max_chars_split() -> None:
    chunker = LayoutChunker(config=ChunkerConfig(max_chars=20))
    page = _make_page([
        _text("Kurzer Text."),        # 12 chars
        _text("Noch ein langer Text der ueber das Limit geht."),  # > 20 chars
    ])
    chunks = chunker.chunk([page])
    assert len(chunks) >= 2
